 ...
 * Now with WebSocketManager to prevent memory leaks
 */

import React, { useState, useEffect, useCallback } from 'react';
import { View, Text, StyleSheet } from 'react-native';
import { colors } from '../theme';
import { wsManager, WSMessageEvent } from '../utils/websocketManager';
...
export function OrderBookDepth({ 
  ...
  const handleWebSocketMessage = useCallback((event: MessageEvent) => {
    ...
    ws.onclose = () => {
      ...
      console.error('OrderBook message parse error:', error);
    }
  }, [levels, minOrderSize]);

  // WebSocket lifecycle management
  useEffect(() => {
    const wsId = `orderbook-${symbol}-${levels}`;
    const wsUrl = `wss://stream.binance.com:9443/ws/${symbol.toLowerCase()}@depth20@100ms`;

    console.log(`[OrderBookDepth] Connecting WebSocket: ${wsId}`);
    
    wsManager.connect(wsId, {
      url: wsUrl,
      onMessage: handleWebSocketMessage,
      onOpen: () => console.log(`[OrderBookDepth] Connected: ${wsId}`),
      onError: (error) => console.error(`[OrderBookDepth] Error: ${wsId}`, error),
      onClose: () => console.log(`[OrderBookDepth] Closed: ${wsId}`),
    });

    return () => {
      console.log(`[OrderBookDepth] Disconnecting WebSocket: ${wsId}`);
      wsManager.disconnect(wsId);
    };
  }, [symbol, levels, handleWebSocketMessage]);
  return (
